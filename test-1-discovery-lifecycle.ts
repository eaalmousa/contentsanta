/**
 * Test 1: Trigger discovery and observe (simplified)
 */
...
// Trigger via API
console.log("\nTriggering discovery via POST /api/topics/${topicId}/run-discovery...");

try {
  const response = await fetch(`http://localhost:5000/api/topics/${topicId}/run-discovery`, {
    method: 'POST',
...
// Wait for processing
console.log("\nWaiting 8 seconds for job to process...");
...
if (afterJobs.rows.length > 0) {
  ...
  if (latestJob.status === 'success' || latestJob.status === 'fail') {
    console.log("\n✅ TEST PASSED: Job reached terminal state");
  ...
  console.log("\n❌ NO DISCOVERY JOBS CREATED - routing is broken");
}
